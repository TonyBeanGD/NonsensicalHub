...
public class RealCut : MonoBehaviour
{
    ...
    private void ClipMesh()
    {
        ...
        SortAngleList.Sort();
        if (SortAngleList.Count>0)
        {
            Vector3 line0to1 = verticeList[rawVerticeCount + 1] - verticeList[rawVerticeCount];
            Debug.Log("first:"+verticeList[rawVerticeCount]);
            foreach (var item in SortAngleList)
            {
                Vector3 line0toi = verticeList[item.Index] - verticeList[rawVerticeCount];
                
                float angle = Vector3.SignedAngle(line0to1.normalized, line0toi.normalized, clipPlane.normal);

                Debug.Log(verticeList[item.Index]+":"+angle);
            }
        }
        #endregion
